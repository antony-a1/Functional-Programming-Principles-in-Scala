...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.map ((p: (Block, Move)) => (p._1, p._2 :: history)).toStream
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter((p : (Block, List[Move])) => !explored.contains(p._1))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        (block, moves) <- initial
        (nextBlock, nextMoves) <- newNeighborsOnly(neighborsWithHistory(block, moves), explored + block)
      } yield (nextBlock, nextMoves)
  ...
  lazy val solutions: Stream[List[Move]] = pathsToGoal map(_._2.reverse)
}
